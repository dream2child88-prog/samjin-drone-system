...
function renderDailyContactReport(){
  ...
  const inactiveWorkers=users.filter(u=>u.role!=='admin').filter(u=>!activeWorkerIds.has(u.id) && !activeWorkerNames.has(u.name)).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'ko'));
...
function handleDialogBeeClick(e){ const move=e.target.closest('[data-bee-change-dialog]'); if(move) return changeBeeVillage(move.dataset.beeChangeDialog, '마을 상세화면에서 담당 마을 변경'); const btn=e.target.closest('[data-bee-done-dialog]'); if(!btn) return; const b=beeFarmers.find(x=>x.id===btn.dataset.beeDoneDialog); if(!b) return; const r=beeRec(b); if(r.done){ if(!confirm('양봉농가 연락완료를 취소할까요?')) return; r.done=false; r.by=''; r.at=''; } else { r.done=true; r.by=currentUser?currentUser.name:'-'; r.at=now(); } save('beeFarmers', beeFarmers); const v=villages.find(x=>x.id===selectedVillageId); if(v) renderDialogBees(v); renderAdmin(); }
