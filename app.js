...
const REMOTE_KEYS = ['users','villages','visibleRounds','uploadHistory','years','activeYear','roundOrder','roundOrderByYear','visibleRoundsByYear'];
...
function remoteSnapshot(){ syncYearContext(); return {users, villages, years, activeYear, roundOrderByYear, visibleRoundsByYear, visibleRounds, uploadHistory, updatedAt: new Date().toISOString()}; }
function applyRemoteSnapshot(data){ if(!data || typeof data !== 'object') return; if(Array.isArray(data.users)) users = data.users; if(Array.isArray(data.years)) years = data.years.map(String); if(data.activeYear) activeYear = String(data.activeYear); if(data.roundOrderByYear && typeof data.roundOrderByYear==='object') roundOrderByYear = data.roundOrderByYear; if(data.visibleRoundsByYear && typeof data.visibleRoundsByYear==='object') visibleRoundsByYear = data.visibleRoundsByYear; if(Array.isArray(data.visibleRounds) && !visibleRoundsByYear[activeYear]) visibleRoundsByYear[activeYear] = data.visibleRounds; syncYearContext(); if(Array.isArray(data.villages)) villages = data.villages.map(migrateVillage); if(Array.isArray(data.uploadHistory)) uploadHistory = data.uploadHistory; localStorage.setItem(LS+'users', JSON.stringify(users)); localStorage.setItem(LS+'villages', JSON.stringify(villages)); localStorage.setItem(LS+'years', JSON.stringify(years)); localStorage.setItem(LS+'activeYear', JSON.stringify(activeYear)); localStorage.setItem(LS+'roundOrderByYear', JSON.stringify(roundOrderByYear)); localStorage.setItem(LS+'visibleRoundsByYear', JSON.stringify(visibleRoundsByYear)); localStorage.setItem(LS+'visibleRounds', JSON.stringify(visibleRounds)); localStorage.setItem(LS+'uploadHistory', JSON.stringify(uploadHistory)); }
...
function bind(){
  ...
  if($('activeYearSelect')) $('activeYearSelect').onchange=changeActiveYear; if($('createYearBtn')) $('createYearBtn').onclick=createNewYear; if($('addRoundBtn')) $('addRoundBtn').onclick=addRound; if($('roundManageTable')) $('roundManageTable').onclick=handleRoundManageClick;
}
...
function renderContact(){ if(selectedRound && !visible().includes(selectedRound)){ selectedRound=''; selectedTown=''; } renderSummary(); if(!selectedRound) return renderRoundView(); if(!selectedTown) return renderTownView(); return renderVillageView(); }
