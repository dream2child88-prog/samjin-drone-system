...
const APP_VERSION = 'v7-daily-report';
...
function renderDailyContactReport(){
  const dateEl=$('dailyReportDate'), roundEl=$('dailyReportRound'), includeEl=$('dailyReportIncludeCanceled'), summaryEl=$('dailyReportSummary'), workerEl=$('dailyReportWorkerTable'), detailEl=$('dailyReportDetailTable');
  ...
  summaryEl.innerHTML=`<div class="daily-stat"><span>연락완료</span><strong>${activeRows.length}</strong></div><div class="daily-stat"><span>참여 방제사</span><strong>${workerRows.length}</strong></div><div class="daily-stat"><span>취소 이력</span><strong>${canceledRows.length}</strong></div>`;
...
function exportDailyContactReport(){
  ...
  const includeCanceled=includeEl ? includeEl.checked : false;
  const rows=collectContactReportRows().filter(x=>x.date===target && (roundFilter==='all'||x.round===roundFilter) && (includeCanceled || !x.canceled));
  const header=['날짜','시간','담당자','차수','읍면','마을','이장님','연락처','상태','취소자','취소시간','취소사유'];
  const body=rows.map(r=>[r.date,r.time,r.worker,r.round,r.eup,r.village,r.chief,r.phone,r.canceled?'취소':'완료',r.cancelBy,r.cancelAt,r.cancelReason]);
